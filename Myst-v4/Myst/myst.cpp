/*
    ...
    1.) Add the included /dependancies/ directory:
        Properties => C\C++ => General => Additional Include Directories: ~/Myst/dependancies/

    2.) Build and Run: Debug x86
    ...
    1   Texture Wrapping Mode: GL_REPEAT
    2   Texture Wrapping Mode: GL_CLAMP_TO_BORDER
...
#include "stb_image.h"  //Load image functions
...
#pragma region Local inclusions

#pragma region GLAD

/*

    OpenGL loader generated by glad 0.1.34 on Sat Feb 27 03:02:38 2021.
...
const float MAX_MOVESPEED = 100.0f;
const float MIN_MOVESPEED = 1.0f;
...
public:
    ...
    //camera options
    float MovementSpeed;
    float StrafeModifier = 1.25f;
    float MouseSensitivity;
    float Zoom;
    bool IsNoClipMode = true;

    //constructor with vectors
    Camera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float yaw = YAW, float pitch = PITCH) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(MoveSpeed), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
    {
        Position = position;
        WorldUp = up;
        Yaw = yaw;
    ...
    Camera(float posX, float posY, float posZ, float upX, float upY, float upZ, float yaw, float pitch) : Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(MoveSpeed), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
    {
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        if (direction == LEFT)
            Position -= Right * velocity * StrafeModifier;
    ...
    void CheckBounds() {
        ...
        //Lock user within bounds
        if (Position.x < Bounds.West) Position.x = Bounds.West;
        else if (Position.x > Bounds.East) Position.x = Bounds.East;
        if (Position.y < Bounds.Bottom) Position.y = Bounds.Bottom;
        else if (Position.y > Bounds.Top) Position.y = Bounds.Top;
        if (Position.z < Bounds.North) Position.z = Bounds.North;
        else if (Position.z > Bounds.South) Position.z = Bounds.South;
    }
    ...
    void ProcessMouseScroll(float yoffset)
    {
        ...
        if (MoveSpeed < MIN_MOVESPEED)
            MoveSpeed = MIN_MOVESPEED;
...
//Window variables
const char* const WINDOW_TITLE = "Final Project: MinimalYST - Ryan DeBraal";
...
//Camera variables
Camera camera(glm::vec3(0.0f, 1.25f, 30.0f));
float mouseX = WINDOW_WIDTH / 2.0f;
float mouseY = WINDOW_HEIGHT / 2.0f;
float orthoScale = 20.0f;
bool isFullscreen = false;
bool isFirstPersonView = true;

//Input variables
int KEY_TOGGLE_FULLSCREEN = GLFW_KEY_F1;
int KEY_TOGGLE_NOCLIPMODE = GLFW_KEY_F2;
int KEY_TOGGLE_PROJECTION = GLFW_KEY_F3;
int KEY_TOGGLE_FLASHLIGHT = GLFW_KEY_F;
...
//Define montain object 
Object3D skybox = {

    glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(200.0f, 200.0f, 200.0f), "clouds",

};

//Define ground object array
Object3D ground[] =
{

    { glm::vec3(-20.0f,    -0.5f,     -30.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(-10.0f,    -0.5f,     -30.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(0.0f,      -0.5f,     -30.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(10.0f,     -0.5f,     -30.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(20.0f,     -0.5f,     -30.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },

    { glm::vec3(-20.0f,    -0.5f,     -20.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(-10.0f,    -0.5f,     -20.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(0.0f,      -0.5f,     -20.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(10.0f,     -0.5f,     -20.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(20.0f,     -0.5f,     -20.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },

    { glm::vec3(-20.0f,    -0.5f,     -10.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(-10.0f,    -0.5f,     -10.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(0.0f,      -0.5f,     -10.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(10.0f,     -0.5f,     -10.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(20.0f,     -0.5f,     -10.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },

    { glm::vec3(-20.0f,    -0.5f,       0.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(-10.0f,    -0.5f,       0.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(0.0f,      -0.5f,       0.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(10.0f,     -0.5f,       0.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(20.0f,     -0.5f,       0.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    ...
    { glm::vec3(-20.0f,    -0.5f,      40.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(-10.0f,    -0.5f,      40.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(0.0f,      -0.5f,      40.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(10.0f,     -0.5f,      40.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
    { glm::vec3(20.0f,     -0.5f,      40.0f),     glm::vec3(10.0f,     0.01f,      10.0f),      "grass",     },
...
Object3D sidewalk[] =
{
    ...
    //Sidewalk
    { glm::vec3(24.1f,    -0.45f,     10.5f),      glm::vec3(0.73f,     0.12f,      2.2f),      "container",     },
    { glm::vec3(22.5f,    -0.45f,     10.3f),      glm::vec3(0.73f,     0.12f,      2.1f),      "container",     },
    { glm::vec3(20.7f,    -0.45f,     10.1f),      glm::vec3(0.74f,     0.12f,      2.3f),      "container",     },
    { glm::vec3(18.4f,    -0.45f,     9.8f),      glm::vec3(0.74f,      0.13f,      2.1f),      "container",     },
    { glm::vec3(16.6f,    -0.45f,     10.0f),      glm::vec3(0.72f,     0.11f,      2.2f),      "container",     },
    { glm::vec3(14.2f,    -0.45f,     10.1f),      glm::vec3(0.72f,     0.13f,      2.0f),      "container",     },
    { glm::vec3(12.2f,    -0.45f,     9.8f),      glm::vec3(0.73f,      0.1f,       2.1f),      "container",     },
    { glm::vec3(10.1f,    -0.45f,     9.9f),      glm::vec3(0.74f,      0.12f,      2.2f),      "container",     },
    { glm::vec3(8.7f,     -0.45f,    10.1f),      glm::vec3(0.72f,      0.11f,      2.4f),      "container",     },
    { glm::vec3(6.4f,     -0.45f,    10.0f),      glm::vec3(0.73f,      0.13f,      2.1f),      "container",     },
    { glm::vec3(3.8f,     -0.45f,    10.1f),      glm::vec3(0.73f,      0.13f,      2.2f),      "container",     },
...
Object3D mountain = {
    ...
    glm::vec3(6.0f, 11.5f, -18.0f), glm::vec3(12.0f, 12.0f, 12.0f), "granite",

};

//Define temple object array
Object3D temple[] =
{

    //Base
    {   glm::vec3(0.0f,      0.0f,      0.0f),       glm::vec3(10.0f,     1.0f,       12.0f),       "concrete",    },

    //Stairs
    {   glm::vec3(0.0f,     -0.12f,      6.2f),       glm::vec3(4.0f,     0.75f,      0.5f),      "concrete",    },
    {   glm::vec3(0.0f,     -0.24f,      6.7f),       glm::vec3(4.0f,     0.5f,       0.5f),      "concrete",    },
    {   glm::vec3(0.0f,     -0.36f,      7.2f),       glm::vec3(4.0f,     0.25f,      0.5f),      "concrete",    },

    //Railing
    {  glm::vec3(-2.25f,     0.0f,      7.0f),       glm::vec3(0.5f,     1.0f,      2.0f),      "concrete",    },
    {   glm::vec3(2.25f,     0.0f,      7.0f),          glm::vec3(0.5f,     1.0f,      2.0f),      "concrete",    },

    //Column 1
    {  glm::vec3(-4.0f,      1.0f,      5.0f),        glm::vec3(1.0f,     1.0f,       1.0f),       "column",    },
    {  glm::vec3(-4.0f,      2.0f,      5.0f),        glm::vec3(1.0f,     1.0f,       1.0f),       "column",    },
    {  glm::vec3(-4.0f,      3.0f,      5.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },

    //Column 2
    {  glm::vec3(-2.0f,      1.0f,      5.0f),        glm::vec3(1.0f,      1.0f,      1.0f),       "column",    },
    {  glm::vec3(-2.0f,      2.0f,      5.0f),        glm::vec3(1.0f,      1.0f,      1.0f),       "column",    },
    {  glm::vec3(-2.0f,      3.0f,      5.0f),        glm::vec3(1.0f,      1.0f,      1.0f),       "column",    },
    ...
    //Walls
    {  glm::vec3(-5.5f,      2.0f,    -1.0f),         glm::vec3(1.0f,     5.0f,      10.0f),        "concrete",    },
    {   glm::vec3(5.5f,      2.0f,    -1.0f),         glm::vec3(1.0f,     5.0f,      10.0f),      "concrete",    },
    {   glm::vec3(0.0f,      2.0f,    -6.5f),         glm::vec3(12.0f,     5.0f,       1.0f),        "concrete",    },

    //Table
    {   glm::vec3(0.0f,     0.75f,      0.0f),        glm::vec3(1.0f,     0.5f,       0.5f),       "container",    },

    //Roof
    {   glm::vec3(0.0f,      4.0f,      0.0f),        glm::vec3(10.0f,     1.00f,      12.0f),      "concrete",    },
...
Object3D roof = {
    ...
    glm::vec3(0.0f, 5.5, -1.5), glm::vec3(12.0f, 2.0f, 11.0f), "concrete",

};

//Define observatory object 
Object3D observatory = {

    glm::vec3(16.0f, 2.0f, 3.8f), glm::vec3(6.0f, 6.0f, 6.0f), "marble",

};

//Define observatory object array
Object3D fountain[] = {

    //Base
    {   glm::vec3(-1.0f,      0.0f,      25.0f),      glm::vec3(0.5f,     1.0f,       1.50f),      "concrete",    },
    {   glm::vec3(1.0f,       0.0f,      25.0f),      glm::vec3(0.5f,     1.0f,       1.50),       "concrete",    },
    {   glm::vec3(0.0f,       0.0f,      24.0f),      glm::vec3(2.5f,     1.0f,       0.5f),       "concrete",    },
    {   glm::vec3(0.0f,       0.0f,      26.0f),      glm::vec3(2.5f,     1.0f,       0.5f),       "concrete",    },

    //Water
    {   glm::vec3(0.0f,       0.0f,      25.0f),      glm::vec3(1.5f,     0.8f,       1.5f),        "water",     },

    //Column 1
    {   glm::vec3(-5.0f,      0.0f,      20.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      1.0f,      20.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      2.0f,      20.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      3.0f,      20.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },

    //Column 2 
    {   glm::vec3(-5.0f,      0.0f,      25.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      1.0f,      25.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      2.0f,      25.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      3.0f,      25.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },

    //Column 3 
    {   glm::vec3(-5.0f,      0.0f,      30.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      1.0f,      30.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      2.0f,      30.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
    {   glm::vec3(-5.0f,      3.0f,      30.0f),        glm::vec3(1.0f,     1.0f,       1.0f),      "column",    },
...
//Stores the data relative to a light in 3D space
const struct PointLight3D {
    glm::vec3 position;
    glm::vec3 ambient;
    glm::vec3 diffuse;
    glm::vec3 specular;
...
//Define light object array
PointLight3D pointLight[] = {

    //Moon
    {
        glm::vec3(0),                    //Position
        glm::vec3(0.9f, 0.9f, 1.0f),     //Ambient 
        glm::vec3(0.9f, 0.9f, 1.0f),     //Diffuse
        glm::vec3(0.9f, 0.9f, 1.0f),     //Specular
        glm::vec3(4.0f),                 //Scale
        false                            //ShowMesh
    },
    ...
    //Fountain                           
    {
        glm::vec3(0.0f, 1.5f, 25.0f),    //Position
    ...
    //Candle                             
    {
        glm::vec3(0.0f, 1.15f,0.0f),     //Position
        glm::vec3(0.8f, 1.0f, 0.0f),     //Ambient 
        glm::vec3(0.8f, 1.0f, 0.0f),     //Diffuse
        glm::vec3(0.8f, 1.0f, 0.0f),     //Specular
        glm::vec3(0.03f, 0.15f, 0.03f),  //Scale
        true                             //ShowMesh
    },
...
{
public:
    enum DayNightState state = DayNightState::DAY_TO_NIGHT;

    glm::vec3 day = glm::vec3(0.95f, 0.95f, 1.0f);
    glm::vec3 night = glm::vec3(0.0f, 0.05f, 0.1f);
    ...
    //Transitions color between day and night cycle
    void Update(float deltaTime)
    {
        //TODO: Use deltaTime for a smooth transition taking into account the framerate

        if (state == DayNightState::DAY_TO_NIGHT) {
            if (r > night.r) r -= increment;
            if (g > night.g) g -= increment;
            if (b > night.b) b -= increment;

            if (r <= night.r && g <= night.g && b <= night.b) {
                //"Night" achieved, stay active for specified duration
                if (current < duration) current += increment; else SwitchState();
            }
        }
        else if (state == DayNightState::NIGHT_TO_DAY) {
            if (r < day.r) r += increment;
            if (g < day.g) g += increment;
            if (b < day.b) b += increment;

            if (r >= day.r && g >= day.g && b >= day.b) {
                //"Day" achieved, stay active for specified duration
                if (current < duration) current += increment; else SwitchState();
            }
        }
    }
...
private:
    void SwitchState() {
        state = (state == DayNightState::DAY_TO_NIGHT) ? DayNightState::NIGHT_TO_DAY : DayNightState::DAY_TO_NIGHT;
        current = 0.0f;
    }
};
DayNightCycle dnc;
...
public:
    ...
    //Transitions value to random float to emulate flickering candle glow
    void Update(float deltaTime)
    {
        //TODO: Use deltaTime for a smooth transition taking into account the framerate
        if (value < target && !isNear(value, target, range)) {
            value += increment;
        }
        else if (value > target && !isNear(value, target, range)) {
            value -= increment;
        }
        else {
...
//Function prototypes
void SetResolution();
void BindKeyPressEvents(GLFWwindow* window, int key, int scancode, int action, int mods);
bool Initialize(int, char* [], GLFWwindow** window);
void UResizeWindow(GLFWwindow* window, int width, int height);
void LoadShaders();
void LoadTextures();
unsigned int LoadTextureFile(const char* path);
void CreateSkyboxMesh(GLMesh& mesh);
void CreateCubeMesh(GLMesh& mesh);
void CreatePyramidMesh(GLMesh& mesh);
void CreateTriangularPrismMesh(GLMesh& mesh);
...
void RenderMountain();
void RenderTemple();
void RenderObservatory();
void RenderFountain();
void RenderTempleRoof();
void RenderPointLights();
void RenderMoon();

//Method which assigns the resolution of the window instance based on whether a flag is set to windowed mode or (fake) fullscreen
void SetResolution() {
    ...
    if (isFullscreen) {
        ...
        glfwSetWindowMonitor(gWindow, nullptr, 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, GLFW_DONT_CARE);
    ...
    else
    {
        WINDOW_WIDTH = 1280;
        WINDOW_HEIGHT = 720;
        int centerX = (videoMode->width / 2) - (WINDOW_WIDTH / 2);
        int centerY = (videoMode->height / 2) - (WINDOW_HEIGHT / 2);
        glfwSetWindowMonitor(gWindow, nullptr, centerX, centerY, WINDOW_WIDTH, WINDOW_HEIGHT, GLFW_DONT_CARE);
        glfwWindowHint(GLFW_DECORATED, true);
    }

    glfwSetWindowSize(gWindow, WINDOW_WIDTH, WINDOW_HEIGHT);
}

//Method which is used to bind the F1 to toggle between windowed mode and (fake) fullscreen
void BindKeyPressEvents(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    if (key == KEY_TOGGLE_FULLSCREEN && action == GLFW_PRESS)
    {
        isFullscreen = !isFullscreen;
        SetResolution();
        cout << "INFO   Resolution: " << (isFullscreen ? "Fullscreen @ " : "Windowed @ ") << WINDOW_WIDTH << "x" << WINDOW_HEIGHT << endl;
    }
    else if (key == KEY_TOGGLE_NOCLIPMODE && action == GLFW_PRESS)
    {
        camera.IsNoClipMode = !camera.IsNoClipMode;
        camera.CheckBounds();
        cout << "INFO   No Clip Mode: " << (!camera.IsNoClipMode ? "On" : "Off") << endl;
    }
    else if (key == KEY_TOGGLE_PROJECTION && action == GLFW_PRESS)
    {
        isFirstPersonView = !isFirstPersonView;
        cout << "INFO   View Mode: " << (!isFirstPersonView ? "Perspective" : "Orthographic") << endl;
    }
    else if (key == KEY_TOGGLE_FLASHLIGHT && action == GLFW_PRESS)
    {
        isFlashlightOn = !isFlashlightOn;
        cout << "INFO   Flashlight: " << (isFlashlightOn ? "On" : "Off") << endl;
    }
...
bool Initialize(int argc, char* argv[], GLFWwindow** window)
{
    ...
    //Tell GLFW to capture mouse input but hide cursor
    glfwSetInputMode(gWindow, GLFW_CURSOR, GLFW_CURSOR_DISABLED);

    //Bind input callbacks to window
    ...
    glfwSetKeyCallback(*window, BindKeyPressEvents);
...
void LoadShaders() {
    lightingShader.Load("shaderfiles/6.multiple_lights.vs", "shaderfiles/6.multiple_lights.fs");
    lightCubeShader.Load("shaderfiles/6.light_cube.vs", "shaderfiles/6.light_cube.fs");
}
...
unsigned int LoadTextureFile(char const* path)
{
...
void CreateSkyboxMesh(GLMesh& mesh)
{
    ...
    //Strides between vertex coordinates is 8 (X, Y, Z, nX, nY, nZ, U, V)
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, stride, (void*)(sizeof(float) * floatsPerVertex));
    glEnableVertexAttribArray(1);

    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, stride, (void*)(sizeof(float) * (floatsPerVertex + floatsPerNormal)));
...
void CreatePyramidMesh(GLMesh& mesh)
{
    //Vertex helper structs
    const struct X_AXIS { float LEFT = -1.0f; float RIGHT = 1.0f; float ZERO = 0.0f; };
    const X_AXIS X;
    const struct Y_AXIS { float BOTTOM = -1.0f; float TOP = 1.0f; float ZERO = 0.0f; };
    const Y_AXIS Y;
    const struct Z_AXIS { float BACK = -1.0f; float FRONT = 1.0f;  float ZERO = 0.0f; };
    const Z_AXIS Z;

    //Position and Color data
    const GLfloat vertices[] = {

        //Triangle 1 - Front              Normals      Texture Coordinates
        X.ZERO,   Y.TOP,     Z.ZERO,      0,0,1,       0,0,
        X.LEFT,   Y.BOTTOM,  Z.FRONT,     0,0,1,       0,1,
        X.RIGHT,  Y.BOTTOM,  Z.FRONT,     0,0,1,       1,0,

        //Triangle 2 - Right              Normals      Texture Coordinates                  
        X.ZERO,   Y.TOP,     Z.ZERO,      1,0,0,       0,0,
        X.RIGHT,  Y.BOTTOM,  Z.FRONT,     1,0,0,       0,1,
        X.RIGHT,  Y.BOTTOM,  Z.BACK,      1,0,0,       1,0,

        //Triangle 3 - Back               Normals      Texture Coordinates                        
        X.ZERO,   Y.TOP,     Z.ZERO,      0,0,-1,      0,0,
        X.RIGHT,  Y.BOTTOM,  Z.BACK,      0,0,-1,      0,1,
        X.LEFT,   Y.BOTTOM,  Z.BACK,      0,0,-1,      1,0,

        //Triangle 4 - Left               Normals      Texture Coordinates                 
        X.ZERO,   Y.TOP,     Z.ZERO,      -1,0,0,      0,0,
        X.LEFT,   Y.BOTTOM,  Z.BACK,      -1,0,0,      0,1,
        X.LEFT,   Y.BOTTOM,  Z.FRONT,     -1,0,0,      1,0,

        //Triangle 5 - Bottom #1          Normals      Texture Coordinates                        
        X.LEFT,   Y.BOTTOM,  Z.BACK,      0,-1,0,      0,0,
        X.RIGHT,  Y.BOTTOM,  Z.BACK,      0,-1,0,      0,1,
        X.LEFT,   Y.BOTTOM,  Z.FRONT,     0,-1,0,      1,0,

        //Triangle 6 - Bottom #2          Normals      Texture Coordinates                 
        X.LEFT,   Y.BOTTOM,  Z.FRONT,     0,-1,0,      1,0,
        X.RIGHT,  Y.BOTTOM,  Z.BACK,      0,-1,0,      0,1,
        X.RIGHT,  Y.BOTTOM,  Z.FRONT,     0,-1,0,      0,0,
    };
    ...
    //Strides between vertex coordinates is 8 (X, Y, Z, nX, nY, nZ, U, V)
    GLint stride = sizeof(float) * (floatsPerVertex + floatsPerNormal + floatsPerUV); //The number of floats before each x coordinate
...
void ProcessInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS && textureWrapMode != GL_REPEAT)
    {
        textureWrapMode = GL_REPEAT;
        cout << "INFO   Texture Wrapping Mode: REPEAT" << endl;
    ...
    else if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS && textureWrapMode != GL_CLAMP_TO_BORDER)
    {
        float color[] = { 1.0f, 0.0f, 1.0f, 1.0f };
        glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, color);

        textureWrapMode = GL_CLAMP_TO_BORDER;
        cout << "INFO   Texture Wrapping Mode: CLAMP TO BORDER" << endl;
...
void UMousePositionCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    float yoffset = (float)(mouseY - ypos); //reversed since y-coordinates go from bottom to top
...
void Update() {
    ...
    //Update per-frame timing variables
    float currentFrame = (float)glfwGetTime();
    gDeltaTime = currentFrame - gLastFrame;
    gLastFrame = currentFrame;

    //Update point light positions
    pointLight[idxMoon].position = glm::vec3(moonOrbitRadius * cos(currentFrame / 12), moonOrbitRadius * sin(currentFrame / 12), -40.0f);
    pointLight[idxSun].position = glm::vec3((sunOrbitRadius * -cos(currentFrame / 12)), (sunOrbitRadius * -sin(currentFrame / 12)), -40.0f);

    //Update candle cycle
    candle.Update(currentFrame);

    //Update day/night cycle
    dnc.Update(gDeltaTime);
}

void Render() {

    //Clear buffer, apply backdrop color
    glClearColor(dnc.r, dnc.g, dnc.b, 1.0f);
    ...
    lightingShader.setVec3("dirLight.ambient", dnc.r, dnc.g, dnc.b);
    lightingShader.setVec3("dirLight.diffuse", 0.0f, 0.0f, 0.0f);
    lightingShader.setVec3("dirLight.specular", 0.0f, 0.0f, 0.0f);

    //Orbitter
    lightingShader.setVec3("pointLights[0].position", pointLight[0].position);
    lightingShader.setVec3("pointLights[0].ambient", pointLight[0].ambient.r, pointLight[0].ambient.g, pointLight[0].ambient.b);
    lightingShader.setVec3("pointLights[0].diffuse", pointLight[0].diffuse.r, pointLight[0].diffuse.g, pointLight[0].diffuse.b);
    lightingShader.setVec3("pointLights[0].specular", pointLight[0].specular.r, pointLight[0].specular.g, pointLight[0].specular.b);
    lightingShader.setFloat("pointLights[0].constant", 1.0f);
    lightingShader.setFloat("pointLights[0].linear", 0.09f);
    lightingShader.setFloat("pointLights[0].quadratic", 0.032f);

    //Fountain
    lightingShader.setVec3("pointLights[1].position", pointLight[1].position);
    lightingShader.setVec3("pointLights[1].ambient", pointLight[1].ambient.r, pointLight[1].ambient.g, pointLight[1].ambient.b);
    lightingShader.setVec3("pointLights[1].diffuse", pointLight[1].diffuse.r, pointLight[1].diffuse.g, pointLight[1].diffuse.b);
    lightingShader.setVec3("pointLights[1].specular", pointLight[1].specular.r, pointLight[1].specular.g, pointLight[1].specular.b);
    lightingShader.setFloat("pointLights[1].constant", 1.0f);
    lightingShader.setFloat("pointLights[1].linear", 0.09f);
    lightingShader.setFloat("pointLights[1].quadratic", 0.032f);

    //Candle
    lightingShader.setVec3("pointLights[2].position", pointLight[2].position);
    lightingShader.setVec3("pointLights[2].ambient", candle.value, candle.value, candle.value);
    lightingShader.setVec3("pointLights[2].diffuse", candle.value, candle.value, candle.value);
    lightingShader.setVec3("pointLights[2].specular", candle.value, candle.value, candle.value);
    lightingShader.setFloat("pointLights[2].constant", 0.1f);
    lightingShader.setFloat("pointLights[2].linear", 0.09f);
    lightingShader.setFloat("pointLights[2].quadratic", 0.032f);

    //Summit
    lightingShader.setVec3("pointLights[3].position", pointLight[3].position);
    lightingShader.setVec3("pointLights[3].ambient", pointLight[3].ambient.r, pointLight[3].ambient.g, pointLight[3].ambient.b);
    lightingShader.setVec3("pointLights[3].diffuse", pointLight[3].diffuse.r, pointLight[3].diffuse.g, pointLight[3].diffuse.b);
    lightingShader.setVec3("pointLights[3].specular", pointLight[3].specular.r, pointLight[3].specular.g, pointLight[3].specular.b);
    ...
    lightingShader.setVec3("spotLight.ambient", (isFlashlightOn) ? 1.0f : 0.0f, (isFlashlightOn) ? 1.0f : 0.0f, (isFlashlightOn) ? 1.0f : 0.0f);
    ...
    if (isFirstPersonView)
        projection = glm::perspective(glm::radians(camera.Zoom), (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT, 0.1f, 500.0f);
    else
        projection = glm::ortho(-((GLfloat)WINDOW_WIDTH / orthoScale), ((GLfloat)WINDOW_WIDTH / orthoScale),
            -((GLfloat)WINDOW_HEIGHT / orthoScale), ((GLfloat)WINDOW_HEIGHT / orthoScale), 0.1f, 500.0f);
    ...
    //Bind skybox mesh VAO
    glBindVertexArray(skyboxMesh.vao);
    RenderSkybox();

    //Bind pyramid mesh VAO
    ...
    //Bind triangular prism mesh VAO
    glBindVertexArray(triangularPrismMesh.vao);
    RenderTempleRoof();
    ...
    //Swap buffers; Flips the the back buffer with the front buffer every frame.
...
void RenderSkybox() {
    ...
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(skybox.position.x, skybox.position.y, skybox.position.z));
    model = glm::scale(model, glm::vec3(skybox.scale.x, skybox.scale.y, skybox.scale.z));
    lightingShader.setMat4("model", model);
...
void RenderGround() {
    //Iterate across array and render meshes
    unsigned int len = sizeof(ground) / sizeof(ground[0]);
    for (unsigned int i = 0; i < len; i++)
    {
        //Calculate the model matrix for each object and pass it to shader before drawing
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(ground[i].position.x, ground[i].position.y, ground[i].position.z));
        model = glm::scale(model, glm::vec3(ground[i].scale.x, ground[i].scale.y, ground[i].scale.z));
        lightingShader.setMat4("model", model);

        //Bind diffuse map
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, textureMap.find(ground[i].texture)->second);
...
void RenderMountain() {
    //Calculate the model matrix for each object and pass it to shader before drawing
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(mountain.position.x, mountain.position.y, mountain.position.z));
    model = glm::scale(model, glm::vec3(mountain.scale.x, mountain.scale.y, mountain.scale.z));
    lightingShader.setMat4("model", model);

    //Bind diffuse map
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, textureMap.find(mountain.texture)->second);
...
void RenderTempleRoof() {
    //TODO: Create roof using a triangular prism mesh

    //Calculate the model matrix for each object and pass it to shader before drawing
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(roof.position.x, roof.position.y, roof.position.z));
    model = glm::scale(model, glm::vec3(roof.scale.x, roof.scale.y, roof.scale.z));
    lightingShader.setMat4("model", model);

    //Bind diffuse map
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, textureMap.find(roof.texture)->second);
...
void RenderSidewalk() {
    //Iterate across array and render meshes
    unsigned int len = sizeof(sidewalk) / sizeof(sidewalk[0]);
    for (unsigned int i = 0; i < len; i++)
    {
        //Calculate the model matrix for each object and pass it to shader before drawing
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(sidewalk[i].position.x, sidewalk[i].position.y, sidewalk[i].position.z));
        model = glm::scale(model, glm::vec3(sidewalk[i].scale.x, sidewalk[i].scale.y, sidewalk[i].scale.z));
        lightingShader.setMat4("model", model);

        //Bind diffuse map
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, textureMap.find(sidewalk[i].texture)->second);
...
void RenderTemple() {
    //Iterate across array and render meshes
    unsigned int len = sizeof(temple) / sizeof(temple[0]);
    for (unsigned int i = 0; i < len; i++)
    {
        //Calculate the model matrix for each object and pass it to shader before drawing
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(temple[i].position.x, temple[i].position.y, temple[i].position.z));
        model = glm::scale(model, glm::vec3(temple[i].scale.x, temple[i].scale.y, temple[i].scale.z));
        lightingShader.setMat4("model", model);

        //Bind diffuse map
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, textureMap.find(temple[i].texture)->second);
...
void RenderObservatory() {
    ...
    //Calculate the model matrix for each object and pass it to shader before drawing
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(observatory.position.x, observatory.position.y, observatory.position.z));
    model = glm::scale(model, glm::vec3(observatory.scale.x, observatory.scale.y, observatory.scale.z));
    ...
    //Bind diffuse map
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, textureMap.find(observatory.texture)->second);
...
void RenderFountain() {
    //Iterate across array and render meshes
    unsigned int len = sizeof(fountain) / sizeof(fountain[0]);
    for (unsigned int i = 0; i < len; i++)
    {
        //Calculate the model matrix for each object and pass it to shader before drawing
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(fountain[i].position.x, fountain[i].position.y, fountain[i].position.z));
        model = glm::scale(model, glm::vec3(fountain[i].scale.x, fountain[i].scale.y, fountain[i].scale.z));
        lightingShader.setMat4("model", model);

        //Bind diffuse map
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, textureMap.find(fountain[i].texture)->second);
...
void RenderPointLights() {
    //Iterate across array and render meshes
    unsigned int len = sizeof(pointLight) / sizeof(pointLight[0]);
    for (unsigned int i = 0; i < len; i++)
    {
        if (!pointLight[i].showMesh)
            continue;

        glm::mat4 model = glm::mat4(1.0f);
        model = glm::mat4(1.0f);
        model = glm::translate(model, pointLight[i].position);
        model = glm::scale(model, glm::vec3(pointLight[i].scale));
...
void RenderMoon() {
    //Calculate the model matrix for each object and pass it to shader before drawing
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(pointLight[idxMoon].position.x, pointLight[idxMoon].position.y, pointLight[idxMoon].position.z));
    model = glm::scale(model, glm::vec3(pointLight[idxMoon].scale.x, pointLight[idxMoon].scale.y, pointLight[idxMoon].scale.z));
    lightingShader.setMat4("model", model);

    //Bind diffuse map
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, textureMap.find("moon")->second);
...
int main(int argc, char* argv[])
{
    ...
    CreateCubeMesh(skyboxMesh);
    CreateCubeMesh(cubeMesh);
    CreatePyramidMesh(pyramidMesh);
    CreateTriangularPrismMesh(triangularPrismMesh);
    CreateLightMesh(lightMesh);
    ...
    DestroyMesh(pyramidMesh);
    DestroyMesh(triangularPrismMesh);
    DestroyMesh(lightMesh);
